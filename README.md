...
Скрипт генерирует html-файл с вакансиями, найденными по указанным параметрам. Файл можно открыть в браузере, результаты отображаются в виде таблицы. Каждую вакансию из таблицы можно открыть на hh.ru, перейдя по ссылке, указанной в последнем столбце. Поиск настраивается с помощью четырех текстовых файлов. Вакансии отсортированы по дате (самые свежие наверху).
